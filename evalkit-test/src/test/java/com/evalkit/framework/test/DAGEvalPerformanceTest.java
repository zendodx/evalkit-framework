 ...
 * DAG模式评测性能测试, 内存占用,耗时
 */
public class DAGEvalPerformanceTest {
    ...
    public static int caseCount = 100000;
    ...
    public static void setUp() {
        String reportName = String.format("DAGEvalPerformanceTest_caseCount_%s", caseCount);
    ...
    public void testPerformance() {
        logger.info("开始DAG模式性能测试，测试用例数量: {}", caseCount);

        // 记录工作流构建前的性能状态
        performanceMonitor.recordMemorySnapshot("工作流构建前");

        EvalTest evalTest = new EvalTest();
        Workflow workflow = evalTest.buildDAGEvalWorkflow(tempDir + fileName);
